...
class BoothsView(viewsets.GenericViewSet):
    ...
    def leaderboard(self, request, *args, **kwargs):
        ...
        #TODO - paginate and maybe ranking?
        try:
            players = Player.objects.all()
            players = players.order_by('score')
            result = serializers.PointSerializers(players, many=True).data
    ...
    def points_name(self, request, *args, **kwargs):
        """return points and name after querying by name. To be used as search filter for leaderboard
        on webpage"""
        try:
            data = self.request.data
            players = Player.objects.get(name__startswith=data.get('name'))
            result = serializers.PointSerializers(players, many=True).data
            return Response(result, status.HTTP_200_OK)
        except Exception as e:
            return Response(e.args, status=status.HTTP_404_NOT_FOUND)
    ...
    def points_rfid(self, request, *args, **kwargs):
        """return points and name after querying by rfid. To be used at the booths if someone wants to check
        their points"""
        try:
            data = self.request.data
